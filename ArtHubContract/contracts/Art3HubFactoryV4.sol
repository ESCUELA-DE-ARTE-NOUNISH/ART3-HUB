...
/**
 * @title Art3Hub Factory V4
 * @dev Factory for creating collections with gasless transactions, subscription integration, and Elite Creator plan
 */
contract Art3HubFactoryV4 is Ownable, ReentrancyGuard, EIP712 {
    ...
    constructor(
    ...
    ) EIP712("Art3HubFactoryV4", "1") Ownable(_initialOwner) {
        ...
        collectionImplementation = address(new Art3HubCollectionV4());
    }
    ...
    function _createCollection(
        ...
        Art3HubCollectionV4(collection).initialize(
            name,
            symbol,
            description,
            image,
    ...
    function mintNFT(
        ...
        // Mint the NFT
        uint256 tokenId = Art3HubCollectionV4(collection).mint(to, tokenURI);
    ...
    function mintNFTGasless(MintVoucher calldata voucher, bytes calldata signature) external nonReentrant {
        ...
        // Mint the NFT
        uint256 tokenId = Art3HubCollectionV4(voucher.collection).mint(voucher.to, voucher.tokenURI);
    ...
    /**
     * @dev Get user's subscription info
     */
    function getUserSubscriptionInfo(address user) external view returns (
